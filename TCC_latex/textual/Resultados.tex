...
\section{Conexões \texttt{SIMILAR\_TO}}
...
\textbf{Grafico com limiar de similaridade e numero de conexões na rede para 
quando wcc é 1. isso para cada período.}

Além da estratégia de limiar adaptativo, foi considerada 
uma segunda configuração para a construção das redes de 
similaridade. Nesse cenário, foi utilizado um limiar fixo 
de similaridade do cosseno igual a 0,72 para todos os 
períodos analisados. Diferentemente do procedimento 
adaptativo, no qual o valor do limiar pode variar entre 
períodos em função da conectividade da rede, a utilização 
de um valor constante mantém inalterado o critério mínimo 
para a formação das relações \texttt{SIMILAR\_TO} ao 
longo de toda a série temporal.

Dessa forma, foram construídas duas versões da rede 
para cada intervalo entre comunicados consecutivos do FOMC. 
Na primeira, denominada neste trabalho de abordagem com limiar 
adaptativo e outra limitada por um limiar fixo para todos os períodos, 
independentemente do número de componentes resultantes.
