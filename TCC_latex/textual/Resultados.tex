...
\begin{figure}[htb]
\centering
\includegraphics[width=\textwidth]{figuras/node_count_by_fomc_period.png}
...
para obter uma rede conectada. Valores mais elevados indicam 
que a conectividade foi alcançada preservando relações 
entre sinais com maior proximidade semântica, enquanto 
valores menores indicam a necessidade de incorporar 
relações entre sinais progressivamente menos semelhantes.

A aplicação desses limiares resultou também em 
diferentes números de relações \texttt{SIMILAR\_TO} entre os períodos. 
A Figura~\ref{fig:relationship_count_by_fomc_period} apresenta a quantidade de conexões 
existentes nas redes após a seleção do limiar correspondente a cada intervalo.

\begin{figure}[htb]
\centering
\includegraphics[width=\textwidth]{figuras/relationship_count_by_fomc_period.png}
\caption{Número de relações \texttt{SIMILAR\_TO} na rede de cada período entre comunicados consecutivos do FOMC.
\newline Fonte: Autoria própria.}
\label{fig:relationship_count_by_fomc_period}
\end{figure}

A quantidade de relações formada em cada período depende tanto 
do número de sinais disponíveis quanto do limiar de similaridade utilizado. 
Assim, períodos com maior número de nós não necessariamente apresentam 
proporcionalmente o mesmo número de conexões, uma vez que a estrutura 
final também depende da proximidade semântica entre os sinais.
